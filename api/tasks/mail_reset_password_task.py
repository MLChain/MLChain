...
def send_reset_password_mail_task(language: str, to: str, code: str):
    ...
    try:
        if language == "zh-Hans":
            ...
            mail.send(to=to, subject="设置您的 Mlchain 密码", html=html_content)
        else:
            html_content = render_template("reset_password_mail_template_en-US.html", to=to, code=code)
            mail.send(to=to, subject="Set Your Mlchain Password", html=html_content)

        end_at = time.perf_counter()
        logging.info(
            click.style("Send password reset mail to {} succeeded: latency: {}".format(to, end_at - start_at), fg="green")
        )
    except Exception:
