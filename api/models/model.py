...
class Conversation(db.Model):
    ...
    def inputs(self):
        ...
        for key, value in inputs.items():
            if isinstance(value, dict) and value.get("mlchain_model_identity") == FILE_MODEL_IDENTITY:
                inputs[key] = File.model_validate(value)
            elif isinstance(value, list) and all(
                isinstance(item, dict) and item.get("mlchain_model_identity") == FILE_MODEL_IDENTITY for item in value
            ):
