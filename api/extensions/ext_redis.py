...
def init_app(app):
    ...
    elif mlchain_config.REDIS_USE_CLUSTERS:
        nodes = [
            ClusterNode(host=node.split(":")[0], port=int(node.split.split(":")[1]))
            for node in mlchain_config.REDIS_CLUSTERS.split(",")
        ]
        redis_client.initialize(RedisCluster(startup_nodes=nodes, password=mlchain_config.REDIS_CLUSTERS_PASSWORD))
    else:
        redis_params.update(
            {
