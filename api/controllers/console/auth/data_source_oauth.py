...
def get_oauth_providers():
    with current_app.app_context():
        notion_oauth = NotionOAuth(
            client_id=mlchain_config.NOTION_CLIENT_ID,
...
class OAuthDataSourceCallback(Resource):
    def get(self, provider: str):
        ...
        if "code" in request.args:
            ...
            return redirect(f"{mlchain_config.CONSOLE_WEB_URL}?type=notion&code={code}")
        elif "error" in request.args:
            error = request.args.get("error")

            return redirect(f"{mlchain_config.CONSOLE_WEB_URL}?type=notion&error={error}")
        else:
            return redirect(f"{mlchain_config.CONSOLE_WEB_URL}?type=notion&error=Access denied")
