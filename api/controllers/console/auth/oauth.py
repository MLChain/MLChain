...
class OAuthCallback(Resource):
    def get(self, provider: str):
        ...
        if invite_token and RegisterService.is_valid_invite_token(invite_token):
            ...
            if invitation:
                ...
                if invitation_email != user_info.email:
                    return redirect(f"{mlchain_config.CONSOLE_WEB_URL}/signin?message=Invalid invitation token.")

            return redirect(f"{mlchain_config.CONSOLE_WEB_URL}/signin/invite-settings?invite_token={invite_token}")

        try:
            account = _generate_account(provider, user_info)
        except AccountNotFoundError:
            return redirect(f"{mlchain_config.CONSOLE_WEB_URL}/signin?message=Account not found.")
        except (WorkSpaceNotFoundError, WorkSpaceNotAllowedCreateError):
            return redirect(
                f"{mlchain_config.CONSOLE_WEB_URL}/signin"
                "?message=Workspace not found, please contact system admin to invite you to join in a workspace."
            )

        # Check account status
        if account.status == AccountStatus.BANNED.value:
            return redirect(f"{mlchain_config.CONSOLE_WEB_URL}/signin?message=Account is banned.")
        ...
        try:
            ...
            return redirect(f"{mlchain_config.CONSOLE_WEB_URL}/signin?message=Workspace not found.")
        ...
        return redirect(
            f"{mlchain_config.CONSOLE_WEB_URL}?access_token={token_pair.access_token}&refresh_token={token_pair.refresh_token}"
        )
