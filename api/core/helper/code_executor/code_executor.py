...
class CodeExecutor:
    ...
    def execute_code(cls, language: CodeLanguage, preload: str, code: str) -> str:
        ...
        :return:
        """
        url = URL(str(mlchain_config.CODE_EXECUTION_ENDPOINT)) / "v1" / "sandbox" / "run"

        headers = {"X-Api-Key": mlchain_config.CODE_EXECUTION_API_KEY}
        ...
        try:
            response = post(
                ...
                headers=headers,
                timeout=Timeout(
                    connect=mlchain_config.CODE_EXECUTION_CONNECT_TIMEOUT,
                    read=mlchain_config.CODE_EXECUTION_READ_TIMEOUT,
                    write=mlchain_config.CODE_EXECUTION_WRITE_TIMEOUT,
                    pool=None,
                ),
            )
