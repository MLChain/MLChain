...
class RelytVectorFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> RelytVector:
        ...
        return RelytVector(
            ...
            config=RelytConfig(
                host=mlchain_config.RELYT_HOST,
                port=mlchain_config.RELYT_PORT,
                user=mlchain_config.RELYT_USER,
                password=mlchain_config.RELYT_PASSWORD,
                database=mlchain_config.RELYT_DATABASE,
            ),
            group_id=dataset.id,
        )
