...
class AIPPTGenerateToolAdapter:
    ...
    def _get_suit(self, style_id: int, colour_id: int) -> int:
        ...
        headers = {
            "x-channel": "",
            "x-api-key": self._tool.runtime.credentials["aippt_access_key"],
            "x-token": self._get_api_token(credentials=self._tool.runtime.credentials, user_id="__mlchain_system__"),
        }
        response = get(
    ...
    def get_runtime_parameters(self) -> list[ToolParameter]:
        ...
        """
        try:
            colors, styles = self.get_styles(user_id="__mlchain_system__")
        except Exception as e:
            colors, styles = (
