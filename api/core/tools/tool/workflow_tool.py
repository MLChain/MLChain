    ...
    def _extract_files(self, outputs: dict) -> tuple[dict, list[File]]:
        ...
        for key, value in outputs.items():
            if isinstance(value, list):
                for item in value:
                    if isinstance(item, dict) and item.get("mlchain_model_identity") == FILE_MODEL_IDENTITY:
                        file = File.model_validate(item)
                        files.append(file)
            elif isinstance(value, dict) and value.get("mlchain_model_identity") == FILE_MODEL_IDENTITY:
