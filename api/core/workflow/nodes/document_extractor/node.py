...
def _extract_text_from_pptx(file_content: bytes) -> str:
    try:
        with io.BytesIO(file_content) as file:
            if mlchain_config.UNSTRUCTURED_API_URL and mlchain_config.UNSTRUCTURED_API_KEY:
                elements = partition_via_api(
                    file=file,
                    api_url=mlchain_config.UNSTRUCTURED_API_URL,
                    api_key=mlchain_config.UNSTRUCTURED_API_KEY,
                )
            else:
